...
public class PaymentPageAccept {
    ...
    public PaymentPageAccept() {
        buyButton.click(); // тестируем функционал "Купить"
        title.get(1).should(ownText("Оплата по карте"));

        // в поле "Номер карты" вводим номер карты со статусом "Одобрена"
        cardNumberField.setValue(DataHelper.getApproved().getNumber());
        ...
        statusField.should(ownText("Операция одобрена Банком."), Duration.ofSeconds(30));
    }
}
